...
/** The band bounds, readable from outside — the spectrum bar draws its eight
 * cells from exactly the bounds the day-count arithmetic uses. */
export function bandBounds(band: TemperatureBand): TempSpan {
  return BAND_BOUNDS[band];
}

/** The eight band captions the spectrum bar sets above its cells. */
export const BAND_LABELS: Record<TemperatureBand, string> = {
  'below-0': '\u2264 0\u00b0',
  '0-5': '0\u20135\u00b0',
  '5-10': '5\u201310\u00b0',
  '10-15': '10\u201315\u00b0',
...
// ---------------------------------------------------------------------------
// THE READER'S OWN PIECES — the read the rebuilt Index tab is drawn from.
//
// The tab lists the reader's LEDGER, not the taxonomy: every count on the
// spectrum bar, every category total and every row is one of these reads.
// A read is pure arithmetic over the piece itself — its stored warmth row
// when one exists, otherwise the same deterministic inference the Today
// pre-filter runs (category + material + name + seasons) — so the tab can
// never show a number that disagrees with the pieces behind it.
// ---------------------------------------------------------------------------

export interface LedgerPieceRead {
  piece: WardrobePiece;
  /** The Index category it belongs to; null when nothing places it. */
  category: GarmentCategoryId | null;
  /** The garment type it answers to, when the keyword read places it. */
  typeId: string | null;
  type: GarmentType | null;
  /** Its own comfortable range — null for a weather-neutral piece. */
  span: TempSpan | null;
  /** The bands that range reaches into. Empty for a neutral piece. */
  bands: TemperatureBand[];
  /** True when the piece is judged by material and place, not temperature. */
  neutral: boolean;
  /** Up to four swatch colours from the piece's own colours. */
  swatches: string[];
  material: string | null;
  /** The maker on the piece, when the reader recorded one. */
  brand: string | null;
}

/** The bands one comfortable range reaches into — the same two-degree
 * overlap rule the band strip and the day counts use. */
function bandsForRange(lo: number, hi: number): TemperatureBand[] {
  const bands: TemperatureBand[] = [];
  for (const def of TEMPERATURE_BANDS) {
    const b = BAND_BOUNDS[def.id];
    if (Math.min(hi, b.hi) - Math.max(lo, b.lo) >= 2) bands.push(def.id);
  }
  return bands;
}

/** ONE ledger piece, read. */
export function readLedgerPiece(
  piece: WardrobePiece,
  warmth: Record<number, PieceWarmth> = {},
  materials: Record<number, string> = {},
): LedgerPieceRead {
  ...
  const lo = read.min_comfortable_temp_c;
  const hi = read.max_comfortable_temp_c;
  const neutral = read.warmth_level === 'all-weather' || !Number.isFinite(lo) || !Number.isFinite(hi) || hi <= lo;
  // The piece's OWN figures, unclamped — the row prints them as the warmth
  // read gives them; only the bar drawing clamps them to the ruler.
  const span = neutral ? null : { lo, hi };
  const swatches: string[] = [];
  for (const c of piece.colors || []) {
    const s = swatchFor(c);
    if (s && swatches.length < 4 && !swatches.includes(s)) swatches.push(s);
  }
  return {
    piece,
    category: pieceIndexCategory(piece),
    typeId,
    type: type && type.category !== 'other' ? type : null,
    span,
    bands: neutral ? [] : bandsForRange(lo, hi),
    neutral,
    swatches,
...
/** The whole ledger, read — newest piece first, as the Ledger lists them. */
export function readLedgerPieces(
  pieces: WardrobePiece[],
  warmth: Record<number, PieceWarmth> = {},
  materials: Record<number, string> = {},
): LedgerPieceRead[] {
  return pieces.map((p) => readLedgerPiece(p, warmth, materials));
}

/** How many of the reader's pieces answer each of the eight bands — the
 * spectrum bar's own figures, never authored. */
export function bandCountsOfReads(reads: LedgerPieceRead[]): Record<TemperatureBand, number> {
  const counts = {} as Record<TemperatureBand, number>;
  for (const band of TEMPERATURE_BAND_ORDER) counts[band] = 0;
  for (const read of reads) for (const band of read.bands) counts[band] += 1;
  return counts;
}

/** The reads bucketed into the eleven categories, in canonical order. A
 * piece nothing places joins no category and is counted separately. */
export function groupReadsByCategory(reads: LedgerPieceRead[]): Record<string, LedgerPieceRead[]> {
  const out: Record<string, LedgerPieceRead[]> = {};
  for (const id of INDEX_CATEGORY_IDS) out[id] = [];
  for (const read of reads) {
    if (read.category && out[read.category]) out[read.category].push(read);
  }
  return out;
}

/** The reads a held band keeps. */
export function readsInBand(reads: LedgerPieceRead[], band: TemperatureBand | null): LedgerPieceRead[] {
  if (!band) return reads;
  return reads.filter((r) => r.bands.includes(band));
}
